$(document).ready(function(){
    ...
    function enemyRandom(num){
        ...
        if((my_num<=35 && check_ok==1)|| (my_num<=35 && first==1 && check_ok !=3)){
   ...
   function suit_check(index_1,index_2){
    ...
    function f_spend_money(selector,index){
        if(used_cards.length<14 || first==1 && used_cards.length<15){
        if(spend_money>money[index]){spend_money=money[index];}
        total_money+= spend_money;
        money[index] -= spend_money;
        if(first==1 && used_cards.length==14){used_cards.length=15; $("#hit").trigger('click');}
        if(money[0]<=0){
            money[0]=0;
